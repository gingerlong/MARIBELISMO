...
\begin{proof}
	...
	Note, by construction of the trees in $\mathcal{T}$ all 0-edges
	are incident to a leaf. 
	By Lemma \ref{lem:contract}, if any tree $(T,\lambda)\in  
	\mathcal{T}$ explains some connected graph $G(\Rl)/\Ro$, 
	then  $(T,\lambda)$ is a least resolved tree that explains 
	$G(\Rl)/\Ro$. However, $\varphi$ maps only those  
	$(T,\lambda)$ to 	some $Q$ for which $T^*\simeq Q$ 
	(cf.\ Lemma \ref{lem:bijection}). 
	Assume there is a least resolved tree $(T',\lambda') \in \mathcal{T}$ that explains $Q$, 
	but the unlabeled tree $T'^*$ obtained from $(T',\lambda')$ 
	by contracting 0-edges is not isomorphic to $Q$. 
