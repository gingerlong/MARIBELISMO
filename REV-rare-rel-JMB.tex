...
\begin{definition}[Least and Minimally Resolved Tree]
  ...
  A pair $(T,\lambda)$ is \emph{minimally resolved} for a prescribed relation
  $\Ro$, $\Rl$ or $\Rld$ if it has the fewest number of vertices among all trees
	that 	explain $\Ro$, $\Rl$ or $\Rld$,
  respectively.
\end{definition}
}
...
\begin{lemma}
	...
	If $Q$ is considered as a graph $G(\Rl)/\Ro$ with vertex set $X$, then 
