...
called \emph{inner vertices}. \rev{In case, $T$ is a single vertex graph
$(\{v\},\emptyset)$, we refer to $v$ as inner vertex and leaf.}
...
\rev{	Let $(T,\lambda)$ be a tree that explains the connected $G(\Rl)/\Ro$. 
...
\begin{thm}
  ...
  \ref{alg:all}, the tree $T^* = T(G(\Rl)/\Ro))$ is 
	\rev{the unique minimally resolved  -- up to choice of inner vertex-- }
%	\mh{I removed unique and minimally -- I don't see it!}
%	\rev{a least} 
%%	the \rev{unique minimally} 
...
\begin{proof}
  ...
  tree that explains $G(\Rl)/\Ro$.
	...
	We show first that for any distinct connected component $Q_i$ and $Q_j$
	the trees $T'(Q_i)$ and $T'(Q_j)$ (induced by the leaf set $V(Q_i)$, resp., $V(Q_j)$)
	don't have any vertex in common. 
	\TODO{proof idea -- take the phylogenetic tree $T'_i$ from $T'(Q_i)$ 
	after contracting all inner 0-edges or 0-edges of degree 2 -- we end in
	unique least resolved tree $T(Q_i)$ (by Thm. \ref{thm:connComp} 2nd statement -- deg2 edges must 
    still be argued). Then old construction of "showing no inner vertex	
	can be identified" can now be re-used. the rest is then  "for free"
	 } 


\mh{ab hier speater weiter*********}


	To this end, we show first, that if $T_1,\dots, T_k$ explains $Q_1,\dots,Q_k$ (possibly not least resolved)
	respectively, then any tree cannot be identified with other tree 
	(due to Thm. \ref{thm:connComp} 2nd statement)
	+ old construction. 

\TODO{	Let $(T,\lambda)$ be the unique minimally resolved tree that explains 
	the connected component $Q$ in $G(\Rl)/\Ro$ and 
	$(T',\lambda')$ be any other tree that explains $Q$. 
	Moreover, let $(T^*,\lambda^*)$ be the tree obtained by 
	contacting all inner 0-edges and all 0-edges
   that are incident to a vertex of
	degree two. 
	inner edges by lemma -- and leaves -- $\lambda^*$.

	Therefore each $T(Q_i)$ is displayed in $T$
	}


	since displayed but no vertices in common they re either connecetd by
	an edge or z-constrauctiom, (adding edge does not work).

 \rev{ First observe that all leaves of the trees $T(Q_i)$ 
%  or the inner vertices
%  of single-vertex trees 
  must be leaves in the tree 
  $T^*$, as otherwise,
  $T^*$ cannot explain $G(\Rl)/\Ro$.  Hence, leaves 
%  or the inner vertices
%  of single-vertex trees 
  of $T(Q_i)$ cannot be identified with any other
  vertex of a distinct tree $T(Q_j)$.  Thus, in order to connect these
  trees inner vertices 
%  (except the  inner vertices of single-vertex
%  trees) 
  must be identified or edges and possibly extra vertices must be
  added. }
  ...
  to $v_i$ and $w_i$ to obtain connectedness in $T^*$, otherwise the leaves $v_i, w_i$
